...
{
    /// <summary>
    /// ViewModel для головного вікна додатку.
    /// </summary>
            ...
            while (true)
            {
                ...
                foreach (Process process in processes)
                {
                    try
                    {
                        ...
                        ProcessInfo infoProcess = new ProcessInfo(process.ProcessName, cpuUsage, memoryUsage);
                ...
                var groupedData = noGroupProcessesList
                    ...
                    .Select(group => new ProcessInfo(
                        ...
                        group.Sum(obj => obj.MemoryUsage))).ToList();
        ...
        public async Task LoggingData()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        newDate = false;
                        var logLines = File.ReadLines("log.txt").ToList();
                    ...
                    using (StreamWriter writer = new StreamWriter("log.txt", !newDate))
                    {
                        newDate = false;
